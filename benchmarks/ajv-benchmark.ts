...
class AjvBenchmark {
  ...
  private loadBaseline(): void {
    const baselinePath = path.join(__dirname, "results", "ajvbaseline.json");
  ...
  private benchmarkSchema(
    ...
    const ajv = new Ajv({ allErrors: false, strict: false });
    addFormats(ajv);
    const ajvAllErrors = new Ajv({
      allErrors: true,
      strict: false,
    });
    addFormats(ajvAllErrors);

    const compilation = this.benchmarkCompilation(schema);

    const validate = ajv.compile(schema);
    const allErrorsValidate = ajvAllErrors.compile(schema);
  ...
  private saveResults() {
    ...
    const resultsPath = path.join(resultsDir, `ajv-${timestamp}.json`);
    const latestPath = path.join(resultsDir, "ajv-latest.json");
  ...
  private generateMarkdownReport() {
    const reportPath = path.join(__dirname, "results", "ajvreport.md");

    let md = "# Ajv Benchmark Report\n\n";
...
if (require.main === module) {
  ...
  const benchmark = new AjvBenchmark();
  benchmark
    .runBenchmarks()
    .then(() => {
      if (saveBaseline) {
