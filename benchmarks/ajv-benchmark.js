...
exports.AjvBenchmark = void 0;
var ajv_1 = require("ajv");
var ajv_formats_1 = require("ajv-formats");
var fs = require("fs");
var path = require("path");
var AjvBenchmark = /** @class */ (function () {
    function AjvBenchmark() {
    ...
    AjvBenchmark.prototype.loadSchemas = function (category) {
        var schemaPath = path.join(__dirname, "schemas", "".concat(category, ".json"));
        return JSON.parse(fs.readFileSync(schemaPath, "utf-8"));
    };
    AjvBenchmark.prototype.loadTestData = function (category) {
        var dataPath = path.join(__dirname, "data", "".concat(category, ".json"));
        return JSON.parse(fs.readFileSync(dataPath, "utf-8"));
    };
    AjvBenchmark.prototype.loadBaseline = function () {
        var baselinePath = path.join(__dirname, "results", "ajvbaseline.json");
    ...
    AjvBenchmark.prototype.measureMemory = function () {
        if (global.gc) {
    ...
    AjvBenchmark.prototype.shuffleArray = function (array) {
        var _a;
    ...
    AjvBenchmark.prototype.benchmark = function (fn, iterations) {
    ...
    AjvBenchmark.prototype.calculateStats = function (values) {
    ...
    AjvBenchmark.prototype.benchmarkMultipleRuns = function (fn, iterations, runs) {
    ...
    AjvBenchmark.prototype.warmup = function (validate, data, iterations) {
        if (iterations === void 0) { iterations = this.warmupIterations; }
        for (var i = 0; i < iterations; i++) {
            validate(data[i % data.length]);
        }
    };
    AjvBenchmark.prototype.benchmarkCompilation = function (schema) {
    ...
    AjvBenchmark.prototype.benchmarkValidation = function (validate, svalidData, sinvalidData, allErrorsValidate) {
        var _this = this;
        var _a;
    ...
    AjvBenchmark.prototype.benchmarkSchema = function (schemaName, schema, validData, invalidData, category) {
        console.log("  Benchmarking: ".concat(schemaName));
        var ajv = new ajv_1.default({ allErrors: false, strict: false });
        (0, ajv_formats_1.default)(ajv);
        var ajvAllErrors = new ajv_1.default({
            allErrors: true,
            strict: false,
        });
        ...
        var allErrorsValidate = ajvAllErrors.compile(schema);
    ...
    AjvBenchmark.prototype.compareWithBaseline = function (result) {
    ...
    AjvBenchmark.prototype.runBenchmarks = function () {
    ...
    AjvBenchmark.prototype.saveResults = function () {
        var resultsDir = path.join(__dirname, "results");
        if (!fs.existsSync(resultsDir)) {
            fs.mkdirSync(resultsDir, { recursive: true });
        }
        var timestamp = new Date().toISOString().replace(/[:.]/g, "-");
        var resultsPath = path.join(resultsDir, "ajv-".concat(timestamp, ".json"));
        var latestPath = path.join(resultsDir, "ajv-latest.json");
        var output = {
    ...
    AjvBenchmark.prototype.printSummary = function () {
        console.log("\n" + "=".repeat(70));
    ...
    AjvBenchmark.prototype.generateMarkdownReport = function () {
        var reportPath = path.join(__dirname, "results", "ajvreport.md");
        var md = "# Ajv Benchmark Report\n\n";
    ...
    AjvBenchmark.prototype.saveAsBaseline = function () {
        var baselinePath = path.join(__dirname, "results", "ajvbaseline.json");
...
exports.AjvBenchmark = AjvBenchmark;
if (require.main === module) {
    var args = process.argv.slice(2);
    var saveBaseline_1 = args.includes("--baseline");
    var benchmark_1 = new AjvBenchmark();
    benchmark_1
        .runBenchmarks()
        .then(function () {
        if (saveBaseline_1) {
